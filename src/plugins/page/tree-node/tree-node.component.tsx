...
class CustomTreeNode extends React.Component<typings.Props, typings.State> {
  ...
  public render() {
    ...
    if (this.pageInfo.type === 'folder') {
      childs = this.pageInfo.childs.map(childKey => {
        return <ConnectedTreeNode key={childKey} pageKey={childKey} />;
      });
    }
    ...
    return <Styled.Container>{React.createElement(TreeNode, childProps, childs)}</Styled.Container>;
  }
  ...
  private handleClick = () => {
    ...
    if (this.pageInfo.type === 'page') {
      ...
      this.props.stores.ApplicationStore.pageInstances.set(
      ...
      if (this.props.stores.ApplicationStore.pageInstances.has(this.props.pageKey)) {
        this.props.actions.ViewportAction.resetViewport(
          this.props.stores.ApplicationStore.pageInstances.get(this.props.pageKey)
        );
      } else {
        // 清空当前视图
