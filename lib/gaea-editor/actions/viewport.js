...
var ViewportAction = function () {
    ...
    (0, _createClass3.default)(ViewportAction, [{
        ...
        value: function onInit() {
            ...
            mobx_1.observe(this.viewport, 'currentEditComponentMapUniqueKey', function (newValue, oldValue) {
                setTimeout(function () {
                    _this.viewport.showEditComponents = !!newValue;
                }, 150);
                var selectClass = 'gaea-selected';
                if (oldValue !== null) {
                    var prevEditDom = _this.viewport.componentDomInstances.get(oldValue);
                    if (dom_1.hasClass(prevEditDom, selectClass)) {
                        dom_1.removeClass(prevEditDom, selectClass);
                    }
                }
                if (newValue !== null) {
                    var nextEditDom = _this.viewport.componentDomInstances.get(newValue);
        ...
        value: function setComponent(mapUniqueKey, componentInfo) {
            ...
            var middlewares = this.viewport.middleware.get('setComponent');
            if (middlewares) {
                middlewares.forEach(function (middleware) {
                    componentInfo = middleware(mapUniqueKey, componentInfo);
                });
            }
            if (!mobx_1.isObservable(componentInfoClone.props)) {
        ...
        value: function addComboComponent(parentMapUniqueKey, componentFullInfo, index) {
            ...
            Object.keys(componentFullInfo.childs).forEach(function (childMapUniqueKey) {
                var expendComponentInfo = _this2.applicationAction.expendComponent(JSON.parse(JSON.stringify(componentFullInfo.childs[childMapUniqueKey])));
                var component = {
                ...
                _this2.setComponent(childMapUniqueKey, component);
            });
        ...
        value: function addComboComponentBySource(parentMapUniqueKey, componentFullInfoSource, index) {
            ...
            Object.keys(componentFullInfoCopy.childs).forEach(function (childKey) {
                componentFullInfoCopy.childs[childKey] = _this3.applicationAction.expendComponent(componentFullInfoCopy.childs[childKey]);
            });
        ...
        value: function removeComponent(mapUniqueKey) {
            ...
            mobx_1.transaction(function () {
                var childMapUniqueKeys = _this4.getAllChildsByMapUniqueKey(mapUniqueKey);
                childMapUniqueKeys.forEach(function (childMapUniqueKey) {
                    _this4.viewport.components.delete(childMapUniqueKey);
                });
                var parentComponentInfo = _this4.viewport.components.get(removeComponentInfo.parentMapUniqueKey);
                ...
                _this4.viewport.components.delete(mapUniqueKey);
                if (mapUniqueKey === _this4.viewport.currentEditComponentMapUniqueKey) {
                    _this4.setCurrentEditComponentMapUniqueKey(null);
                }
                if (mapUniqueKey === _this4.viewport.currentHoverComponentMapUniqueKey) {
                    _this4.setCurrentHoverComponentMapUniqueKey(null);
                }
            });
        }
    }, {
        ...
        value: function completionEditProps(componentProps) {
            ...
            var middlewares = this.viewport.middleware.get('completionEditProps');
            if (middlewares) {
                middlewares.forEach(function (middleware) {
        ...
        value: function middlewareRegister(viewportFunctionName, func) {
            if (!this.viewport.middleware.has(viewportFunctionName)) {
                ...
                var funcs = this.viewport.middleware.get(viewportFunctionName);
