...
var Viewport = function () {
    function Viewport(application) {
        ...
        this.currentEditPropsIndex = null;
        this.currentEditIsWeb = false;
        this.currentEditEventIndex = 0;
    ...
    (0, _createClass3.default)(Viewport, [{
        ...
        value: function setComponents(mapUniqueKey, componentInfo) {
            ...
            if (componentInfo.parentMapUniqueKey === null) {
                componentInfo.props.gaeaEdit = componentInfo.props.gaeaEdit.filter(function (edit) {
        ...
        value: function completionEditProps(componentProps) {
            if (!componentProps.gaeaEventData) {
                componentProps.gaeaEventData = mobx_1.observable([]);
            }
            if (!componentProps.gaeaNativeEventData) {
                componentProps.gaeaNativeEventData = mobx_1.observable([]);
            }
            if (!componentProps.gaeaVariables) {
                componentProps.gaeaVariables = mobx_1.observable([]);
            }
        ...
        key: "setIsMovingComponent",
        value: function setIsMovingComponent(isMoving) {
        ...
        value: function prepareWriteHistory(mapUniqueKey) {
            var componentInfo = this.components.get(mapUniqueKey || this.currentEditComponentMapUniqueKey);
            this.oldProps = JSON.parse(JSON.stringify(componentInfo.props));
        }
    }, {
        key: "writeHistory",
        value: function writeHistory(mapUniqueKey) {
            var componentInfo = this.components.get(mapUniqueKey || this.currentEditComponentMapUniqueKey);
            var newValue = JSON.parse(JSON.stringify(componentInfo.props));
            this.saveOperate({
                type: 'update',
        ...
        key: "updateAbsoluteXY",
        value: function updateAbsoluteXY(mapUniqueKey, x, y) {
            var componentInfo = this.components.get(mapUniqueKey);
        ...
        key: "addVariable",
        value: function addVariable(mapUniqueKey, variableInfo) {
            var componentInfo = this.components.get(mapUniqueKey);
            var existIndex = componentInfo.props.gaeaVariables.findIndex(function (variable) {
                return variable.field === variableInfo.field;
            });
            if (existIndex > -1) {
        ...
        key: "removeVariable",
        value: function removeVariable(mapUniqueKey, field) {
            var componentInfo = this.components.get(mapUniqueKey);
            var existIndex = componentInfo.props.gaeaVariables.findIndex(function (variable) {
                return variable.field === field;
            });
            if (existIndex > -1) {
                componentInfo.props.gaeaVariables.splice(existIndex, 1);
            }
        ...
        key: "findComponentsByName",
        value: function findComponentsByName(name) {
            var components = [];
        ...
        key: "getNamesWithCount",
        value: function getNamesWithCount() {
            var nameMap = new Map();
            this.components.forEach(function (component) {
            ...
            nameMap.forEach(function (count, name) {
                nameArray.push({ count: count, name: name });
            });
            return nameArray;
        }
    }, {
        ...
        value: function setCurrentEditPropsIndex(index, eventProps, currentEditIsWeb, eventIndex) {
            ...
            if (index !== null) {
                ...
                componentProps = this.completionEditProps(mobx_1.extendObservable({}, this.application.expendComponentProps(eventProps)));
            } else {
                componentProps = this.completionEditProps(mobx_1.extendObservable({}, this.application.expendComponentProps(this.temporaryOriginProps)));
                this.temporaryOriginProps = null;
            }
...
__decorate([mobx_1.action('设置是否在拖拽中')], Viewport.prototype, "setIsMovingComponent", null);
...
__decorate([mobx_1.action('新增一个变量配置')], Viewport.prototype, "addVariable", null);
__decorate([mobx_1.action('删除一个变量配置')], Viewport.prototype, "removeVariable", null);
__decorate([mobx_1.observable], Viewport.prototype, "currentEditPropsIndex", void 0);